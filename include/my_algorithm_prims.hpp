...
void MyAlgorithmPrims(graph::WeightedGraph<WeightType>& graph, std::vector<std::vector<WeightType>>* resEdges) {

    const int INF = 1000000000;
    std::vector<int> vertices(graph.NumVertices());
    std::vector<std::vector<WeightType>> edges(graph.NumVertices() - 1, std::vector<WeightType>(2));
    std::vector<int> minEdge(int(graph.NumVertices()), INF), selEdge (int(graph.NumVertices()), -1);
    minEdge[0] = 0;
